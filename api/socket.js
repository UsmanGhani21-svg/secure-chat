...
export const config = {
  api: {
    bodyParser: false, // ❌ Required for WebSockets
  },
};
  ...
  if (!res.socket.server.io) {
    ...
    io.on("connection", (socket) => {
      ...
      socket.on("setupProfile", (data) => {
        ...
        conn.profile = { displayName, anonymousId };
        conn.lastActivity = Date.now();
      ...
      socket.on("sendMessage", ({ roomId, encryptedMessage, messageId }) => {
        const conn = activeConnections.get(socket.id);
        const room = activeRooms.get(roomId);
        if (!conn?.isAuthenticated || !room?.participants.has(socket.id)) return;

        socket.to(roomId).emit("newMessage", {
          messageId,
          roomId,
          sender: conn.profile?.displayName || "Anonymous",
          encryptedPayload: encryptedMessage,
          timestamp: Date.now(),
        });
        socket.emit("messageDelivered", { messageId, delivered: true });
      ...
      socket.on("shareFile", ({ roomId, encryptedFile, fileMetadata, fileId }) => {
        const conn = activeConnections.get(socket.id);
        const room = activeRooms.get(roomId);
        if (!conn?.isAuthenticated || !room?.participants.has(socket.id))
          return socket.emit("fileShareFailed", { error: "Not allowed", fileId });
